...
function switchTab(tabKey) {
  ...
  if (tabKey === 'group5') {
    ...
    if (title) title.textContent = `${tabKey.toUpperCase()} — Weekly Import`;
...
function validateTradingWeek(startISO, endISO) {
  ...
  if (end < start) throw new Error('The Summary end date is before the start date.');
  if (start.getDay() !== 1 || end.getDay() !== 5 || diff !== 4) {
    throw new Error(
...
async function analyzeWeeklyImport() {
  ...
  if (!summaryFile) {
    showToast('Choose the weekly MT5 Summary HTML first.', 'error');
    return;
  }
  ...
  setImportStatus('Reading weekly Summary and Coverage History…', 'working');
  ...
  try {
    ...
    if (analyzeButton) {
      ...
      analyzeButton.textContent = 'Analyze Week';
    }
  }
}
...
function renderImportPreview() {
  ...
  preview.innerHTML = `
    <div class="import-summary-grid">
      <div class="import-stat"><span>DETECTED WEEK</span><strong>${escapeHTML(pendingImport.weekLabel)}</strong></div>
    ...
    <div class="preview-grid">
      ${rankingCard('Weekly Top 5 Winners', pendingImport.winners, 'winner', false)}
      ${rankingCard('Weekly Top 5 Losers', pendingImport.losers, 'loser', false)}
    </div>
    ...
    ${renderAccountsSection(pendingImport.accounts, 'All Weekly Accounts', 'preview')}
...
function renderUnmatchedDeals(rows) {
  ...
  return `
    <div class="warning-panel">
      ...
      <p>No P/L is guessed. Re-import the same week with a longer Coverage History if older opening comments are missing.</p>
...
function renderCoverageAuditSection(matchedDeals, unmatchedDeals) {
  ...
  if (!rows.length) {
    return `
      <details class="audit-panel">
        ...
        <div class="empty-state">No OUT coverage deals were closed inside this week.</div>
      </details>`;
  }
  ...
  return `
    <details class="audit-panel">
      <summary>Coverage Deal Audit · ${rows.length} weekly OUT deals</summary>
      <div class="table-scroll tall-scroll">
        <table class="matrix-table compact-table">
...
async function saveWeeklyImport() {
  ...
  const existingSnapshot = await db.ref(`pl_weekly_store/${currentBranch}/${pendingImport.weekKey}`).once('value');
  ...
  const updates = {};
  updates[`pl_weekly_store/${currentBranch}/${pendingImport.weekKey}`] = record;
...
function resetImportUI(clearFiles = true) {
  ...
  const saveButton = document.getElementById('save-week-import-btn');
  if (saveButton) saveButton.disabled = true;
  setImportStatus('Upload the weekly Summary and Coverage History.', 'neutral');
  renderExistingWeekStatus();
}
...
function renderExistingWeekStatus() {
  ...
  if (pendingImport) {
    const existing = weeklyStore?.[currentBranch]?.[pendingImport.weekKey];
    el.innerHTML = existing
      ? `<span class="status-badge check">RE-IMPORT</span><strong>${escapeHTML(pendingImport.weekLabel)}</strong><small>Existing saved week will be replaced only after confirmation.</small>`
      : `<span class="status-badge good">NEW WEEK</span><strong>${escapeHTML(pendingImport.weekLabel)}</strong><small>Ready to save after analysis.</small>`;
    return;
  }

  const records = Object.values(weeklyStore?.[currentBranch] || {}).sort((a, b) => String(b.weekStart || '').localeCompare(String(a.weekStart || '')));
  if (!records.length) {
    el.innerHTML = '<span class="status-badge neutral">NO DATA</span><small>No weekly reports saved yet.</small>';
    return;
  }
...
function renderBranchWeekLedger() {
  ...
  if (!records.length) {
    container.innerHTML = '<div class="empty-state"><strong>No weekly reports saved.</strong> Your first Friday import will appear here permanently.</div>';
    ...
    <div class="table-card">
      <div class="table-scroll">
        <table class="matrix-table ledger-table">
          <thead><tr><th>WEEK</th><th>STATUS</th><th>ACCOUNTS</th><th>CLIENT P/L</th><th>COVER PROFIT</th><th>BROKER NET</th><th>UNMATCHED</th><th>SAVED</th><th></th></tr></thead>
          <tbody>
            ${records.map(record => {
              ...
              return `
                <tr class="${record.key === selectedBranchWeekKey ? 'selected-row' : ''}">
                  ...
                  <td><span class="status-badge ${needsCheck ? 'check' : 'good'}">${needsCheck ? 'CHECK' : 'COMPLETE'}</span></td>
  ...
  container.innerHTML = `
    <section class="saved-detail">
      <div class="section-heading">
        <div><div class="eyebrow">SAVED WEEK · ${escapeHTML(currentBranch.toUpperCase())}</div><h2>${escapeHTML(record.weekLabel || weekKey)}</h2></div>
    ...
    Object.entries(weeklyStore?.[branch] || {}).forEach(([weekKey, record]) => {
      if (!map[weekKey]) {
        map[weekKey] = {
          ...
          weekLabel: record.weekLabel || weekKey,
          branches: []
        };
      }
...
function renderManagementView() {
  ...
  if (!catalog.length) {
    ...
    if (select) select.innerHTML = '<option value="">No saved weeks</option>';
    ...
    if (combined) combined.innerHTML = '<div class="empty-state wide"><strong>No weekly imports yet.</strong> Saved branch weeks will automatically appear here.</div>';
      ...
      if (!result.record) {
        return `
          <div class="branch-pair-block">
            ...
            <div class="empty-state compact">No report saved for this week.</div>
          </div>`;
      }
      ...
      return `
        <div class="branch-pair-block">
          <div class="branch-pair-title">
            ...
            <span class="status-badge ${Number(result.record.stats?.unmatchedCoverDeals || 0) ? 'check' : 'good'}">${Number(result.record.stats?.unmatchedCoverDeals || 0) ? 'CHECK' : 'COMPLETE'}</span>
          </div>
...
function renderExecutiveSourceStatus(weekKey) {
  ...
  if (!weekKey) {
    note.innerHTML = '<span class="status-dot neutral"></span>No weekly branch reports saved yet.';
    return;
  }
...
function renderArchiveView() {
  ...
  const catalog = getWeekCatalog();
  if (!catalog.length) {
    list.innerHTML = '<p class="subtitle" style="padding:10px;">No saved weeks yet.</p>';
    detail.innerHTML = '';
...
function renderArchiveDetail(weekKey) {
  ...
  container.innerHTML = `
    <div class="section-heading">
      <div><div class="eyebrow">SAVED TRADING WEEK</div><h2>${escapeHTML(catalogItem.weekLabel)}</h2></div>
      <div class="section-note">Permanent weekly record</div>
    </div>
...
async function clearEverything() {
  ...
  const firstConfirm = window.confirm(
    'CLEAR EVERYTHING will permanently delete ALL saved weekly reports, coverage mappings, and legacy P/L records for this system.\n\nThis cannot be undone. Continue?'
  );
  ...
  try {
    ...
    if (activePanel?.id === 'view-group5') {
      ...
      renderBranchWeekLedger();
      renderExistingWeekStatus();
      setImportStatus('All saved P/L data has been cleared. Upload a new weekly report when ready.', 'neutral');
    }
